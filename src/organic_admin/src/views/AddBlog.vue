   ...
   <div class="py-4 container-fluid">
      <div class="card">
         <div class="card-header pb-0">
            <div class="row">
               <div class="col-md-12">
                  <h6 class="mb-0 text-uppercase">Thêm bài viết</h6>
               </div>
            </div>
         </div>
...
export default {
   ...
   methods: {
         ...
         return API.post(
            apiPath + `/${apiGroup}/add.php`,
            {
               ...
               content: this.$refs.quillEditorRef.getHTML(),
         		systemAdminId: this.$store.state.accountLogin.id
            },
            (data) => {
               if (data.code === 1) {
                  ElMessage({
	...
	mounted() {
		const systemAdminNickname = this.$store.state.accountLogin.nickname;
		this.$refs.systemAdminNicknameRef.setValue(systemAdminNickname);
	},
};
</script>
