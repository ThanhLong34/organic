...
function getList($limit, $offset, $searchByName)
{
   ...
   $baseQuery = "SELECT * FROM `tag` WHERE `DeletedAt` IS NULL";
   ...
   if ($result) {
      ...
      $response = new ResponseAPI(1, "Lấy danh sách thẻ thành công", $tags);
      $response->send();
   } else {
      $response = new ResponseAPI(2, "Lấy danh sách thẻ thất bại");
      $response->send();
   }
