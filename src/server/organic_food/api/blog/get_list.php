...
$searchType = trim($_GET["searchType"] ?? ""); // Hợp lệ: title
...
function getList($limit, $offset, $searchType, $searchValue, $fillType, $fillValue, $orderby, $reverse)
{
   ...
   $baseQuery = "SELECT `$tableName`.*, `image`.`link` AS 'featureImageUrl', `systemadmin`.`nickname` AS 'systemAdminNickname'
      FROM `$tableName` 
